...
# set timeout to 1 ms (prior to v 4.x.x timeout=1)
uart = io.UART(board.TX, board.RX, baudrate=31250, timeout=0.001)
...
while True:
    ...
    for n in range(knob_count):
        ...
        # send out CC code and value for MIDI chan 0
        data = bytearray([0xB0, n, int(cc_range[n].range_slicer(knob[n].value))])
        uart.write(data)
        # print(data)
    time.sleep(0.01)
